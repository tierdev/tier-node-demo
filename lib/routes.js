...
routes.get('/payment', log, mustLogin, async (req, res) => {
  ...
  const customerID = await tier.whois(`org:${user}`)
...
const getStripeCustomer = async (customerID) => {
  ...
  const c = await stripeRes.json()
  console.error('CUSTOMER OBJ', c)
  return c
}
...
routes.get('/attach-payment-method', log, mustLogin, async (req, res) => {
  ...
  if (redirect_status === 'succeeded' && seti.payment_method) {
    // attach to customer
    const customerID = await tier.whois(`org:${req.cookies.user}`)
...
routes.post('/convert', log, mustLogin, async (req, res) => {
  ...
  await tier.report(`org:${user}`, 'feature:convert') // TK
...
routes.post('/plan', log, mustLogin, async (req, res) => {
  const { plan } = req.body
  const user = req.cookies.user
  console.error('SUBSCRIBE', { org: `org:${user}`, plan })
  console.error(await tier.subscribe(`org:${user}`, plan))
