...
routes.post('/convert', mustLogin, async (req, res) => {
  ...
  if (C !== undefined) {
    return res.send({ F: cToF(C), currentUsage: await tier.currentUsage(`org:${user}`, 'feature:convert') })
  } else {
    return res.send({ C: fToC(F), currentUsage: await tier.currentUsage(`org:${user}`, 'feature:convert')  })
  }
})
