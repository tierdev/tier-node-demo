...
routes.post('/plan', mustLogin, async (req, res) => {
  ...
  await tier.appendPhase(`org:${user}`, plan)
...
routes.post('/convert', mustLogin, async (req, res) => {
  ...
  const reservation = await tier.reserve(user, 'feature:convert', 1)

  // if they're not allowed, then cancel the reservation and send 402
  if (!reservation.ok) {
    await reservation.cancel()
    return res.status(402).send({ error: 'not allowed by plan' })
  }

  // uhoh, this is an error, we need to cancel the reservation, because
  // we aren't going to be able to deliver the feature that they requested.
